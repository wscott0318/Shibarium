...
const getClient = async (clientType = "pos") => {
  if (clientType === "pos") {
    return getPOSClient()
  }
  return getPlasmaClient()
}

const loadContract = async (path, abi, layer = "l1", type = "plasma") => {
  const client = await getPOSClient()
  const abiJson = await client.client.abiManager.getABI(abi, type)
  const contractAddress = utils.isAddress(path) ? path : client.client.abiManager.getConfig(path)
  let contractObj

  if (layer === "l1") {
    contractObj = await client.client.parent.getContract(contractAddress, abiJson)
